 ...
 *   buffer.put...
 */
class AdvancedBuffer extends EventEmitter {
  ...
  _digest(buffer, ...args) {
    ...
    if (buffer.length > retVal) {
      this.emit('data', buffer.subarray(0, retVal), ...args);

      // note that each chunk has probably different length
      this._nextLength = 0;

      // digest buffer recursively
      return this._digest(buffer.subarray(retVal), ...args);
    }
  }

}

module.exports = AdvancedBuffer